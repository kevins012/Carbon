...
function getTransactionToken(parameter) {
   return snap.createTransaction(parameter) // Mengembalikan Promise
       .then((transaction) => {
           let transactionToken = transaction.token; // Ambil token
           return transactionToken; // Kembalikan token
       })
       .catch((error) => {
           console.error('Error creating transaction:', error);
           throw error; // Tangani error dengan baik
       });
}
// async function transaction() {
//    try {
//       const transaction = await getTransactionToken(parameter)
//       return transaction;
